...
function requiredFieldCheck() {
    ...
    resetError();
    if ($('#id_title').val() === '') {
        $('#id_errorResponse').text("Please add a Title to this assignment to proceed. Title, Course, and DueDate are required fields.");
        $("#id_errorResponseMain").show();
        setTimeout(function() { $("#id_errorResponseMain").hide(); }, timeout);
        return
    }
    if ($('#id_course').val() === '') {
        $('#id_errorResponse').text("Please add a Course to this assignment to proceed. Title, Course, and DueDate are required fields.");
        $("#id_errorResponseMain").show();
        setTimeout(function() { $("#id_errorResponseMain").hide(); }, timeout);
        return
    }
    if ($('#id_due_by').val() === '') {
        $('#id_errorResponse').text("Please add a DueDate to this assignment to proceed. Title, Course, and DueDate are required fields.");
        $("#id_errorResponseMain").show();
        setTimeout(function() { $("#id_errorResponseMain").hide(); }, timeout);
        return
    }
